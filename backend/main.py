...
@app.get("/api/page_render/{upload_id}/{num}/{gen}")
def get_page_render(upload_id: str, num: int, gen: int) -> Response:
    """Render the PDF page that owns content stream *num* and return a PNG."""
    import fitz  # pymupdf — imported lazily so startup isn't delayed

    doc = _sessions.get(upload_id)
    if doc is None:
        raise HTTPException(404, "Session not found")

    # Find the Page object that references this stream via /Contents (or /Contents[n])
    backref_index = _backref_cache.get(upload_id, {})
    page_obj_num: int | None = None
    for ref in backref_index.get(num, []):
        kp = ref["key_path"]
        if kp == "Contents" or kp.startswith("Contents."):
            ...
            if candidate is not None:
                ...
                if tp.is_name() and tp.sval == "Page":
                    page_obj_num = ref["from_num"]
                    break

    if page_obj_num is None:
        raise HTTPException(404, "No page references this content stream")

    # Walk the page tree to find the 0-based page index
    def _page_index(page_target: int) -> int:
        root_ref = doc._trailer.get("Root")
        if not root_ref.is_ref():
            return -1
        catalog = doc.resolve_num(root_ref.ref.num, root_ref.ref.gen)
        if catalog is None:
            return -1
        ...
        def walk(node_num: int) -> bool:
            node = doc.resolve_num(node_num)
            if node is None:
                return False
            tp = node.get("Type")
            if tp.is_name() and tp.sval == "Pages":
                kids = node.get("Kids")
                if kids.is_array():
                    for kid_ref in kids.arr:
                        if kid_ref.is_ref():
                            if walk(kid_ref.ref.num):
                                return True
            elif tp.is_name() and tp.sval == "Page":
                if node_num == page_target:
                    result[0] = counter[0]
                    return True
                counter[0] += 1
            return False

        walk(pages_ref.ref.num)
        return result[0]

    page_index = _page_index(page_obj_num)
    if page_index < 0:
        raise HTTPException(404, "Could not determine page index")

    # Find the PDF file on disk (saved under uploads/<upload_id>/)
    upload_dir = _UPLOADS_DIR / upload_id
    ...
    try:
        ...
        mat = fitz.Matrix(1.5, 1.5)  # 1.5× scale for a crisp render
        pix = page.get_pixmap(matrix=mat)
        png_data = pix.tobytes("png")
        fitz_doc.close()
    except Exception as exc:
        raise HTTPException(500, f"Page rendering failed: {exc}") from exc
